<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>

<div id="company-container">
	<h2 id="company-title">객실 상세</h2>
	<div id="company-img-div">
		<span><img id="company-img"
			src="https://file.mk.co.kr/meet/neds/2022/03/image_readtop_2022_252256_16476552624979285.jpg"
			alt="11" style="max-width: 50%; height: auto;"></span>
	</div>
	<div>
		<hr>
		<div>신라호텔</div>
		<p></p>

		<div id="Appstar-rated" class="star-rated">
			<span id="spanstar-ratedFirst" title="5">☆</span> <span
				id="spanstar-ratedsecond" title="4">☆</span> <span
				id="spanstar-ratedthird" title="3">☆</span> <span
				id="spanstar-ratedFourth" title="2">☆</span> <span
				id="spanstar-ratedFifth" title="1">☆</span>
		</div>
		<div>
			리뷰 : <input type="hidden">251개
		</div>
		<p></p>
		<div></div>
		<p></p>
		<hr>
		<div>
			<span>객실안내/예약 : <input type="hidden"></span>
		</div>
		<div>
			<span>주소 : <input type="hidden"></span>
		</div>
		<div>
			<span>전화번호 : <input type="hidden"></span>
		</div>
		<div>
			<span>지도 : <input type="hidden"></span>
		</div>
		<hr>
		<div>
			<div id="Appstar-rated" class="star-rated">
				<span id="spanstar-ratedFirst" title="5">☆</span> <span
					id="spanstar-ratedsecond" title="4">☆</span> <span
					id="spanstar-ratedthird" title="3">☆</span> <span
					id="spanstar-ratedFourth" title="2">☆</span> <span
					id="spanstar-ratedFifth" title="1">☆</span>
			</div>
			<div>
				<span>객실후기(39) <input type="hidden"></span>
			</div>
		</div>
		<div>
			포토리뷰만 보기<input type="checkbox">
		</div>
		<hr>
		<hr>
		<div>
			<div id="Appstar-rated" class="star-rated">
				<span id="spanstar-ratedFirst" title="5">☆</span> <span
					id="spanstar-ratedsecond" title="4">☆</span> <span
					id="spanstar-ratedthird" title="3">☆</span> <span
					id="spanstar-ratedFourth" title="2">☆</span> <span
					id="spanstar-ratedFifth" title="1">☆</span>
			</div>
			<div>
				<span>닉네임</span> | <span>날짜</span>
			</div>
			<div>
				<span>객실명 <input type="hidden"></span>&nbsp <span>스탠다드<input
					type="hidden"></span>
			</div>
			<p></p>
			<div>
				<span>내용내용<input type="hidden"></span>
				<hr>
				<div id="Appstar-rated" class="star-rated">
					<span id="spanstar-ratedFirst" title="5">☆</span> <span
						id="spanstar-ratedsecond" title="4">☆</span> <span
						id="spanstar-ratedthird" title="3">☆</span> <span
						id="spanstar-ratedFourth" title="2">☆</span> <span
						id="spanstar-ratedFifth" title="1">☆</span>
				</div>
				<div>
					<span>닉네임<input type="hidden"></span> | <span>날짜<input
						type="hidden"></span>
				</div>
				<div>
					<span>객실명<input type="hidden"></span>&nbsp <span>스탠다드<input
						type="hidden"></span>
				</div>
				<p></p>
				<div>
					<span>내용내용<input type="hidden"></span>
				</div>
			</div>
		</div>
	</div>