
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Document</title>
</head>
<body>
	<div id="company-container">
		<h2 id="company-title">객실 상세</h2>
		<div id="company-img-div">
			<span><img id="company-img"
				src="https://file.mk.co.kr/meet/neds/2022/03/image_readtop_2022_252256_16476552624979285.jpg"
				alt="11" style="max-width: 50%; height: auto;"></span>
		</div>
		<div>
			<hr>
			<div class="comp-title">신라호텔</div>
			<p></p>

			<div id="Appstar-rated" class="star-rated">
				<span id="spanstar-ratedFirst" title="5">☆</span> <span
					id="spanstar-ratedsecond" title="4">☆</span> <span
					id="spanstar-ratedthird" title="3">☆</span> <span
					id="spanstar-ratedFourth" title="2">☆</span> <span
					id="spanstar-ratedFifth" title="1">☆</span>
			</div>
			<div class="comp-review">
				리뷰 : <input type="hidden">251개
			</div>
			<span>
				<button class="comp-call-icon">전화</button>
			</span>
			<div></div>
			<div></div>
			<div></div>
			<hr>
			<div>
				<span class="comp-room">객실안내/예약 : <input type="hidden"></span>
			</div>
			<div>
				<span class="comp-addrs">주소 : <input type="hidden"></span>
			</div>
			<div>
				<span class="comp-phone">전화번호 : <input type="hidden"></span>
			</div>
			<div>
				<span class="comp-map">지도 : <input type="hidden"></span>
			</div>
			<hr>
			<div>
				<div id="Appstar-rated" class="star-rated">
					<span id="spanstar-ratedFirst" title="5">☆</span> <span
						id="spanstar-ratedsecond" title="4">☆</span> <span
						id="spanstar-ratedthird" title="3">☆</span> <span
						id="spanstar-ratedFourth" title="2">☆</span> <span
						id="spanstar-ratedFifth" title="1">☆</span>
				</div>
				<div>
					<span class="comp-room-review">객실후기(39) <input type="hidden"></span>
				</div>
			</div>
			<div class="comp-photo-review">
				포토리뷰만 보기<input type="checkbox">
			</div>
			<hr>
			<hr>
			<div>
				<div id="Appstar-rated" class="star-rated">
					<span id="spanstar-ratedFirst" title="5">☆</span> <span
						id="spanstar-ratedsecond" title="4">☆</span> <span
						id="spanstar-ratedthird" title="3">☆</span> <span
						id="spanstar-ratedFourth" title="2">☆</span> <span
						id="spanstar-ratedFifth" title="1">☆</span>
				</div>
				<div>
					<span class="comp-nickName">닉네임</span> | <span
						class="comp-reviewDate">날짜</span>
				</div>
				<div>
					<span>객실명 <input type="hidden"></span>&nbsp <span
						class="comp-roomName">스탠다드<input type="hidden"></span>
				</div>
				<p></p>
				<div>
					<span class="">내용내용<input type="hidden"></span>
				</div>
				<hr>
				<div id="Appstar-rated" class="star-rated">
					<span id="spanstar-ratedFirst" title="5">☆</span> <span
						id="spanstar-ratedsecond" title="4">☆</span> <span
						id="spanstar-ratedthird" title="3">☆</span> <span
						id="spanstar-ratedFourth" title="2">☆</span> <span
						id="spanstar-ratedFifth" title="1">☆</span>
				</div>
				<div>
					<span class="comp-nickName">닉네임</span> | <span
						class="comp-reviewDate">날짜</span>
				</div>
				<div>
					<span>객실명 <input type="hidden"></span>&nbsp <span
						class="comp-roomName">스탠다드<input type="hidden"></span>
				</div>
				<p></p>
				<div>
					<span class="">내용내용<input type="hidden"></span>
				</div>
			</div>
		</div>
	</div>
</body>
</html>
