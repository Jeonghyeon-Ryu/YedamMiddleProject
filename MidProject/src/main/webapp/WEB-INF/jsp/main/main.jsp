<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>

<main>
<template>
	<div class="card" style="max-width:540px">
        <div class="row">
            <div class="col-md-8">
                <img src="" class="img-fluid rounded" alt="...">
            </div>
            <div class="col-md-4">
                <div class="card-body">
                    <h5 class="card-title"></h5>
                    <div class="star-rated">
                        <img src="img/star-empty-20.png" class="first-star">
                        <img src="img/star-empty-20.png" class="second-star">
                        <img src="img/star-empty-20.png" class="third-star">
                        <img src="img/star-empty-20.png" class="fourth-star">
                        <img src="img/star-empty-20.png" class="fifth-star">
                    </div>
                    <p class="card-address"><strong>지역 :</strong></p>
                    <p class=""><strong>가격 :</strong></p>
                    <p class="card-text"><small class="text-muted">Last updated</small></p>
                </div>
            </div>
        </div>
    </div>
</template>
    <div class="card" style="max-width:540px">
        <div class="row">
            <div class="col-md-8">
                <img src="https://a0.muscache.com/im/pictures/373443ec-b377-4181-b753-3a2f3508c2b3.jpg?im_w=720" class="img-fluid rounded" alt="...">
            </div>
            <div class="col-md-4">
                <div class="card-body">
                    <h5 class="card-title">Card title</h5>
                    <div class="star-rated">
                        <img src="img/star-empty-20.png" class="first-star">
                        <img src="img/star-empty-20.png" class="second-star">
                        <img src="img/star-empty-20.png" class="third-star">
                        <img src="img/star-empty-20.png" class="fourth-star">
                        <img src="img/star-empty-20.png" class="fifth-star">
                    </div>
                    <p><strong>지역 :</strong> 대구광역시</p>
                    <p><strong>가격 :</strong> 500,000원 </p>
                    <p class="card-text"><small class="text-muted">Last updated 3 mins ago</small></p>
                </div>
            </div>
        </div>
    </div>
    <div class="card" style="max-width:540px">
        <div class="row">
            <div class="col-md-8">
                <img src="https://a0.muscache.com/im/pictures/373443ec-b377-4181-b753-3a2f3508c2b3.jpg?im_w=720" class="img-fluid rounded" alt="...">
            </div>
            <div class="col-md-4">
                <div class="card-body">
                    <h5 class="card-title">Card title</h5>
                    <div class="star-rated">
                        <img src="img/star-empty-20.png" class="first-star">
                        <img src="img/star-empty-20.png" class="second-star">
                        <img src="img/star-empty-20.png" class="third-star">
                        <img src="img/star-empty-20.png" class="fourth-star">
                        <img src="img/star-empty-20.png" class="fifth-star">
                    </div>
                    <p><strong>주소 :</strong> 대구광역시</p>
                    <p><strong>가격 :</strong> 500,000원 </p>
                    <p class="card-text"><small class="text-muted">Last updated 3 mins ago</small></p>
                </div>
            </div>
        </div>
    </div>
    <div class="card" style="max-width:540px">
        <div class="row">
            <div class="col-md-8">
                <img src="https://a0.muscache.com/im/pictures/373443ec-b377-4181-b753-3a2f3508c2b3.jpg?im_w=720" class="img-fluid rounded" alt="...">
            </div>
            <div class="col-md-4">
                <div class="card-body">
                    <h5 class="card-title">Card title</h5>
                    <div class="star-rated">
                        <img src="img/star-empty-20.png" class="first-star">
                        <img src="img/star-empty-20.png" class="second-star">
                        <img src="img/star-empty-20.png" class="third-star">
                        <img src="img/star-empty-20.png" class="fourth-star">
                        <img src="img/star-empty-20.png" class="fifth-star">
                    </div>
                    <p><strong>주소 :</strong> 대구광역시</p>
                    <p><strong>가격 :</strong> 500,000원 </p>
                    <p class="card-text"><small class="text-muted">Last updated 3 mins ago</small></p>
                </div>
            </div>
        </div>
    </div>
    <div class="card" style="max-width:540px">
        <div class="row">
            <div class="col-md-8">
                <img src="https://a0.muscache.com/im/pictures/373443ec-b377-4181-b753-3a2f3508c2b3.jpg?im_w=720" class="img-fluid rounded" alt="...">
            </div>
            <div class="col-md-4">
                <div class="card-body">
                    <h5 class="card-title">Card title</h5>
                    <div class="star-rated">
                        <img src="img/star-empty-20.png" class="first-star">
                        <img src="img/star-empty-20.png" class="second-star">
                        <img src="img/star-empty-20.png" class="third-star">
                        <img src="img/star-empty-20.png" class="fourth-star">
                        <img src="img/star-empty-20.png" class="fifth-star">
                    </div>
                    <p><strong>주소 :</strong> 대구광역시</p>
                    <p><strong>가격 :</strong> 500,000원 </p>
                    <p class="card-text"><small class="text-muted">Last updated 3 mins ago</small></p>
                </div>
            </div>
        </div>
    </div>
    <div class="card" style="max-width:540px">
        <div class="row">
            <div class="col-md-8">
                <img src="https://a0.muscache.com/im/pictures/373443ec-b377-4181-b753-3a2f3508c2b3.jpg?im_w=720" class="img-fluid rounded" alt="...">
            </div>
            <div class="col-md-4">
                <div class="card-body">
                    <h5 class="card-title">Card title</h5>
                    <div class="star-rated">
                        <img src="img/star-empty-20.png" class="first-star">
                        <img src="img/star-empty-20.png" class="second-star">
                        <img src="img/star-empty-20.png" class="third-star">
                        <img src="img/star-empty-20.png" class="fourth-star">
                        <img src="img/star-empty-20.png" class="fifth-star">
                    </div>
                    <p><strong>주소 :</strong> 대구광역시</p>
                    <p><strong>가격 :</strong> 500,000원 </p>
                    <p class="card-text"><small class="text-muted">Last updated 3 mins ago</small></p>
                </div>
            </div>
        </div>
    </div>
</main>