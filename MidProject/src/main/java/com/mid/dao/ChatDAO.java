...
public class ChatDAO extends DAO {
	...
	public Chat selectLastContent(String myId,String youId) {
		connect();
		String sql = "select *from(select content,receiver,sender,checkemoji,currenttime,to_char(currenttime,'HH24:MI')ct,chat_seq from chat where (sender =? and receiver =?) or (sender =? and receiver = ?) order by chat_seq desc) where ROWNUM=1";
		Chat vo = new Chat();
		try {
